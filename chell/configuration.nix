...
in {
  ...
  services.udev.packages = [pkgs.yubikey-personalization pkgs.openrgb pkgs.logitech-udev-rules pkgs.nrf-udev];
  ...
  users.users.bs2k = {
    isNormalUser = true;
    extraGroups = ["wheel" "adbusers" "docker" "dialout" "tss" "libvirtd" "video" "render" "input"]; # Enable ‘sudo’ for the user.
