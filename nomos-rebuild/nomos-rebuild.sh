...
copyToTarget() {
    if ! [ "$targetHost" = "$buildHost" ]; then
        if [ -z "$targetHost" ]; then
            ...
            NIX_SSHOPTS=$SSHOPTS runCmd nix-copy-closure "${copyFlags[@]}" --from "$buildHost" "$1" --log-format internal-json -v |& nom --json
        elif [ -z "$buildHost" ]; then
            logVerbose "Running nix-copy-closure with these NIX_SSHOPTS: $SSHOPTS"
            NIX_SSHOPTS=$SSHOPTS runCmd nix-copy-closure "${copyFlags[@]}" --to "$targetHost" "$1" --log-format internal-json -v |& nom --json
        else
            buildHostCmd nix-copy-closure "${copyFlags[@]}" --to "$targetHost" "$1" --log-format internal-json -v |& nom --json
        fi
    fi
}
...
nixBuild() {
    ...
    if [ -z "$buildHost" ]; then
        ...
        runCmd nix-build "$@" --log-format internal-json -v |& nom --json
    else
        ...
        if [ -a "$drv" ]; then
            logVerbose "Running nix-copy-closure with these NIX_SSHOPTS: $SSHOPTS"
            NIX_SSHOPTS=$SSHOPTS runCmd nix-copy-closure --to "$buildHost" "$drv" --log-format internal-json -v |& nom --json
            buildHostCmd nix-store -r "$drv" "${buildArgs[@]}" --log-format internal-json -v |& nom --json
        else
...
nixFlakeBuild() {
    ...
    then
        runCmd nix "${flakeFlags[@]}" build "$@" --log-format internal-json -v |& nom --json
        readlink -f ./result
    elif [ -z "$buildHost" ]; then
        runCmd nix "${flakeFlags[@]}" build "$@" --out-link "${tmpDir}/result" --log-format internal-json -v |& nom --json
        ...
        if [ -a "$drv" ]; then
            ...
            NIX_SSHOPTS=$SSHOPTS runCmd nix "${flakeFlags[@]}" copy "${copyFlags[@]}" --derivation --to "ssh://$buildHost" "$drv" --log-format internal-json -v |& nom --json
            buildHostCmd nix-store -r "$drv" "${buildArgs[@]}" --log-format internal-json -v |& nom --json
        else
            log "nix eval failed"
            exit 1
...
if [[ -n $upgrade && -z $_NIXOS_REBUILD_REEXEC && -z $flake ]]; then
    ...
    for channelpath in /nix/var/nix/profiles/per-user/root/channels/*; do
        ...
        if [[ "$channel_name" == "nixos" ]]; then
            runCmd nix-channel --update "$channel_name" --log-format internal-json -v |& nom --json
...
# Commented out as this would result in the nom wrapping getting lost
...
if [ -z "$rollback" ]; then
    ...
    if [[ "$action" = switch || "$action" = boot ]]; then
        ...
        targetHostCmd nix-env -p "$profile" --set "$pathToConfig" --log-format internal-json -v |& nom --json
